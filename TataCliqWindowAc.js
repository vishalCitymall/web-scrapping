...
const tataCliqWindowAC = express();
  ...
  try {
    ...
    for (let pageNumber = 0; pageNumber < 2; pageNumber++) {
      let res = await axios.get(
        `https://prodsearch.tatacliq.com/products/mpl/search/?searchText=%3Arelevance%3Acategory%3AMSH1230101%3AinStockFlag%3Atrue&isKeywordRedirect=true&isKeywordRedirectEnabled=false&channel=WEB&isMDE=true&isTextSearch=false&isFilter=false&qc=false&test=mm.new&page=${pageNumber}&isSuggested=false&isPwa=true&pageSize=40&typeID=all`
      );
    ...
    const collectionName = "window_AC";
...
//find()
tataCliqWindowAC.get("/", (req, res) => {
  res.send("hello world");
});

tataCliqWindowAC.listen(port);
